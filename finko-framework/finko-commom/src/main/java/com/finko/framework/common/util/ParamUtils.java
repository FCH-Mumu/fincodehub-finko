...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checkFinkoId(String finkoId) {
        // 检查长度
        if (finkoId.length() < ID_MIN_LENGTH || finkoId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(finkoId).matches();
    }
