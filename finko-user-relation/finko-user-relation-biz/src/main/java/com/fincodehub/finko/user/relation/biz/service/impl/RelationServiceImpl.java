package com.fincodehub.finko.user.relation.biz.service.impl;
...
import com.fincodehub.finko.user.relation.biz.enums.LuaResultEnum;
import com.fincodehub.finko.user.relation.biz.enums.ResponseCodeEnum;
...
import com.fincodehub.finko.user.relation.biz.rpc.UserRpcService;
import com.fincodehub.finko.user.relation.biz.service.RelationService;
...
public class RelationServiceImpl implements RelationService {
  ...
  @Resource private FollowingMapper followingMapper;
   ...
   */
  @Override
  public ResponseObject<?> unfollow(UnfollowUserReqVO unfollowUserReqVO) {
    // 想要取关了用户 ID
    ...
    if (Objects.equals(result, LuaResultEnum.ZSET_NOT_EXIST.getCode())) { // ZSET 关注列表不存在
      // 从数据库查询当前用户的关注关系记录
      List<Following> followingDOS = followingMapper.selectByUserId(userId);
   ...
   */
  @Override
  public ResponseObject<?> follow(FollowUserReqVO followUserReqVO) {
    ...
    // 当前登录的用户 ID
    Long userId = LoginUserContextHolder.getUserId();
    log.info("==> relation 当前登录的用户 userId: {}", userId);
    // 校验：无法关注自己
    ...
    log.info("==> 开始发送关注操作 MQ, 主题: {}", destination);
    String hashKey = String.valueOf(userId);
    // 异步发送 MQ 消息，提升接口响应速度
  ...
  private static void checkLuaScriptResult(Long result) {
    ...
    log.info("==> relation Lua 脚本返回结果: {}", luaResultEnum);
  ...
  public PageResponse<FindFollowingUserRspVO> findFollowingList(FindFollowingListReqVO findFollowingListReqVO) {
    ...
    if (total > 0) { // 缓存中有数据
      ...
      // 分页查询
      List<Following> followingDOS = followingMapper.selectPageListByUserId(userId, offset, limit);
      // 赋值真实的记录总数
      total = count;
      ...
      if (CollUtil.isNotEmpty(followingDOS)) {
        // 提取所有关注用户 ID 到集合中
        List<Long> userIds = followingDOS.stream().map(Following::getFollowingUserId).toList();
  ...
  private void syncFollowingList2Redis(Long userId) {
    // 查询全量关注用户列表（1000位用户）
    List<Following> followingDOS = followingMapper.selectAllByUserId(userId);
    if (CollUtil.isNotEmpty(followingDOS)) {
      // 用户关注列表 Redis Key
      String followingListRedisKey = RedisKeyConstants.buildUserFollowingKey(userId);
      // 随机过期时间
  ...
  public PageResponse<FindFansUserRspVO> findFansList(FindFansListReqVO findFansListReqVO) {
    ...
    if (total > 0) { // 缓存中有数据
      ...
      // 分页查询
      List<Fans> fansDOS = fansDOMapper.selectPageListByUserId(userId, offset, limit);

      // 若记录不为空
      if (CollUtil.isNotEmpty(fansDOS)) {
        // 提取所有粉丝用户 ID 到集合中
        List<Long> userIds = fansDOS.stream().map(Fans::getFansUserId).toList();
  ...
  private void syncFansList2Redis(Long userId) {
    // 查询粉丝列表（最多5000位用户）
    List<Fans> fansDOS = fansDOMapper.select5000FansByUserId(userId);
    if (CollUtil.isNotEmpty(fansDOS)) {
      // 用户粉丝列表 Redis Key
      String fansListRedisKey = RedisKeyConstants.buildUserFansKey(userId);
      // 随机过期时间
